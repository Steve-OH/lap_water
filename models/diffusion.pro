...
// This is the relative humidity at which to run the simulation. The value is used to compute
// both the equilibrium moisture content in the wood and the diffusivity of water vapor in the
// lumens (cell cavities) of the wood. Note that this is given as a fraction, rather than as a
// percentage.

  Hsim = 0.55;  // typical outdoor relative humidity for SE Ohio
...
Function {
  ...
  Tsim[] = FtoK[52];  // 52°F is the year-round average outdoor temperature for SE Ohio
...
// cellulose/lignin).
  ...
  Gsim = 0.72;  // typical value for white oak
...
Function {
  ...
  Uemc[] = (1800 / W[]) * (K[] * Hsim / (1 - K[] * Hsim)
         ...
         / (1 + K1[] * K[] * Hsim + K1[] * K2[] * K[] * K[] * Hsim * Hsim));     // Wood10 (4-5)
...
Constraint {
  ...
  { Name Sta_u;
    Case {
      { Region Sur_WiW; Type Assign; Value Uemc[]; }  // fix surface moisture content
    }
  }
}
...
Tstart      =  0;            // start time
Tstep_init  =  1 * days;     // initial (trial) time step
